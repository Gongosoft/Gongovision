...
async function send_mark_read_queue(queue, asRead, callback) {
	...
	for (let i = queue.length - 1; i >= 0; i--) {
		...
		if (div.classList.contains('not_read')) {
			...
			div.querySelectorAll('a.read > .icon').forEach(function (img) {
				img.outerHTML = myIcons.read;
			});
			inc--;
...
function mark_read(div, only_not_read, asBatch) {
	if (!div || !div.id || context.anonymous || (only_not_read && !div.classList.contains('not_read'))) {
		return false;
	}
...
function mark_favorite(div) {
	...
	req.onload = function (e) {
		...
		div.querySelectorAll('a.bookmark').forEach(function (a) {
			a.href = json.url;
		});
		div.querySelectorAll('a.bookmark > .icon').forEach(function (img) {
			img.outerHTML = json.icon;
		});
...
function loadLazyImages(rootElement) {
	rootElement
		.querySelectorAll('img[data-original], iframe[data-original], video[data-original], track[data-original]')
		.forEach(function (el) {
			if (el.tagName === 'VIDEO') {
...
function toggleContent(new_active, old_active, skipping) {
	...
	if (context.sticky_post) {
		// Stick the article to the top when opened
		...
		if (
			nav_menu &&
			(getComputedStyle(nav_menu).position === 'fixed' || getComputedStyle(nav_menu).position === 'sticky')
		) {
...
function show_share_menu(el) {
	...
	if (!dropdownMenu) {
		...
		const template = document
			...
			.innerHTML.replace(/--entryId--/g, id)
			.replace(/--link--/g, link)
...
async function mylabels(key) {
	...
	if (scrollTop !== 0) {
		if (mylabelsDropdown.closest('.horizontal-list.flux_header')) {
			mylabelsDropdown.nextElementSibling.nextElementSibling.scrollIntoView({
				behavior: 'smooth',
				block: 'start'
			});
		} else {
...
function onScroll() {
	...
	if (context.auto_mark_scroll) {
		...
		document.querySelectorAll('.not_read:not(.keep_unread)').forEach(function (div) {
			if (div.offsetHeight > 0 && div.offsetParent.offsetTop + div.offsetTop + div.offsetHeight < minTop) {
				mark_read(div, true, true);
			}
		});
	}
...
function init_posts() {
	if (context.auto_load_more || context.auto_mark_scroll || context.auto_remove_article) {
		box_to_follow =
			context.current_view === 'global' ? document.getElementById('panel') : document.scrollingElement;
		...
		if (a) {
			...
			if (!dropdownMenu) {
				...
				const template = document
					...
					.replace('http://example.net/', feed_web);
				div.insertAdjacentHTML('beforeend', template);
			...
			window.addEventListener(
				...
				() => {
					...
					const navbarHeight =
						document.querySelector('nav#nav_entries')?.getBoundingClientRect()?.height || 0;
...
function init_shortcuts() {
	...
	document.addEventListener('keydown', (ev) => {
		...
		if (k === s.skip_next_entry) {
			next_entry(true);
			ev.preventDefault();
			return;
		}
		if (k === s.skip_prev_entry) {
			prev_entry(true);
			ev.preventDefault();
			return;
		}
		if (k === s.collapse_entry) {
			collapse_entry();
			ev.preventDefault();
			return;
		}
		...
		if (k === s.close_menus && ((hash === 'slider' && close_slider_listener()) || hash.startsWith('dropdown'))) {
			location.hash = 'close';
			ev.preventDefault();
			return;
		}
		if (k === s.help) {
			window.open(context.urls.help);
			ev.preventDefault();
			return;
		}
		if (k === s.focus_search) {
		...
		if (k === s.normal_view) {
			delayedClick(document.querySelector('#nav_menu_views .view-normal'));
			ev.preventDefault();
			return;
		}
		if (k === s.reading_view) {
			delayedClick(document.querySelector('#nav_menu_views .view-reader'));
			ev.preventDefault();
			return;
		}
		if (k === s.global_view) {
			delayedClick(document.querySelector('#nav_menu_views .view-global'));
			ev.preventDefault();
			return;
		}
		if (k === s.toggle_media) {
...
function init_stream(stream) {
	stream.onclick = function (ev) {
		...
		if (el) {
			// Allow default control/command-click behaviour such as open in background-tab
		...
		if (el) {
			...
			if (ev.target.tagName.toUpperCase() === 'A') {
				// Leave real links alone (but does not catch img in a link)
				if (context.auto_mark_article) {
	...
	stream.onmouseup = function (ev) {
		// Mouseup enables us to catch control+click in IE/Edge
	...
	stream.onauxclick = function (ev) {
		// Auxclick enables us to catch middle click
	...
	if (nav_entries) {
		...
		nav_entries.querySelector('.up').onclick = function (e) {
			...
			document.scrollingElement.scrollTop =
				windowTop > item_top ? item_top - nav_menu_height : 0 - nav_menu_height;
			return false;
		};
	}
}
...
async function loadDynamicTags(div) {
	div.querySelectorAll('li.item').forEach(function (li) {
		li.remove();
	});
	...
	if (!context.anonymous) {
		...
		input_newTag.addEventListener('keydown', function (ev) {
		...
		button_btn.addEventListener('click', function () {
		...
		req.onload = function (e) {
			...
			if (json.categories.length + json.feeds.length > 0 && !document.getElementById('actualizeProgress')) {
				document.body.insertAdjacentHTML(
					'beforeend',
					'<div id="actualizeProgress" class="notification good">' +
					json.feedback_actualize +
					'<br /><span class="title">/</span><br /><span class="progress">0 / ' +
					to_process +
					'</span></div>'
				);
			} else {
...
function notifs_html5_show(nb, nb_new) {
	...
	try {
		const notification = new window.Notification(context.i18n.notif_title_articles, {
			...
			body:
				context.i18n.notif_body_new_articles.replace('%%d', nb_new) +
				' ' +
				context.i18n.notif_body_unread_articles.replace('%%d', nb),
...
function refreshUnreads() {
	...
	req.onload = function (e) {
		...
		Object.keys(json.feeds).forEach(function (feed_id) {
			...
			if (
				(incUnreadsFeed(null, feed_id, nbUnreads - feed_unreads) || isAll) && // Update of current view?
...
function load_more_posts() {
	...
	req.onload = function (e) {
		...
		streamAdopted.querySelectorAll('.flux, .transition').forEach(function (div) {
			if (
				lastTransition !== null &&
				div.classList.contains('transition') &&
				div.textContent === lastTransition.textContent
			) {
				lastTransition = null;
...
function init_confirm_action() {
	...
	if (slider) {
		slider.addEventListener('freshrss:slider-load', function (e) {
			slider.querySelectorAll('button.confirm').forEach(function (b) {
				b.disabled = false;
			});
		});
	}
